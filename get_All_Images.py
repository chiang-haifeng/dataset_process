...
if __name__ == '__main__':
    ...
    dir_path = "D:/workspace/dataset_process/dataset/CASIA-Iris-Thousand"
    ...
    # 3.写入文件中（txt）todo full_file_name为完整的图片路径，bri_file_name为只包含文件名的图片路径
    full_file_name = os.path.join("D:/workspace/dataset_process", 'full-' + dir_path.split('/')[-1] + '.txt').replace("\\",'/')
    bri_file_name = os.path.join("D:/workspace/dataset_process", dir_path.split('/')[-1] + '.txt').replace("\\",'/')
    print('包含路径和文件名的txt文件为{}'.format(full_file_name))
    print('仅包含文件名的的txt文件为{}'.format(bri_file_name))
    # full_file为完整的图片路径，bri_file为只包含文件名的图片路径
    full_file = open(full_file_name, "w")
    bri_file = open(bri_file_name, "w")
    #遍历原列表，写入文件
    for item in file_path_list:
        full_file.write(item + '\n')
        bri_file.write(item.split('/')[-1] + '\n')
